...
function App() {
  ...
  function handleDragEnter(e, index, innerIndex) {
    ...
    setList((prev) => {
      ...
      newList[index].items.splice(
        innerIndex,
        0,
        newList[currentIndex].items.splice(currentInnerIndex, 1)[0]
      );
      draggedItem.current = [index, innerIndex];
      return newList;
    });
        ...
        {list.map((item, index) => {
          return (
            <div
              className="col"
              onDragEnter={
                isDragging && !item.items.length
                  ? (e) => handleDragEnter(e, index, 0)
                  : null
              }
              key={index}
            >
              ...
              {item.items.map((innerItem, innerIndex) => {
                return (
                  <div
                    className={
                      isDragging ? getStyles(index, innerIndex) : "item"
                    }
                    draggable
                  ...
                  >
                    ...
                    <button
                      className="close-btn"
                      onClick={() => deleteItem(index, innerIndex)}
                    >
                      X
                    </button>
                  </div>
